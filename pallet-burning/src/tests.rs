...
fn test_burn() {
    new_test_ext().execute_with(|| {
        assert_ok!(BurningModule::burn_tft(Origin::signed(alice()), 900000000000, "some_message".as_bytes().to_vec()));
...
fn test_burn_to_much_fails() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            BurningModule::burn_tft(Origin::signed(alice()), 1200000000000, "some_message".as_bytes().to_vec()),
